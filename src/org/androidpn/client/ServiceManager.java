	...
	public void setTags(final List<String> tagsList){
		...
		//因为涉及到网络请求，应当异步进行
				new Thread(new Runnable() {
					...
					public void run() {
						try {
							Thread.sleep(1000);//为了等待notificationService先启动起来
						...
						if(xmppManager != null){
							...
							Log.d(LOGTAG, "authenticaticated already.send SetTagsIQ now...");
							// 组装IQ
							...
							// 发送IQ，但是不能通过
							// xmppManager.getConnection().sendPacket(deliverConfirmIQ);的方式发送，因为先有ServiceManager才有xmppManager
							// 难点是得不到xmppManager实例,可以通过notificationService拿到
							xmppManager.getConnection().sendPacket(iq);//发送出去，但是此时时机不对，因为有可能Client 和 Service还没连接认证完成
					//建立等待的机制，等待身份认证成功，在XmppManager.java中的心跳线程启动的后面解锁（因为此时身份认证肯定完成了）
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				if(xmppManager != null){
					...
					Log.d(LOGTAG, "authenticaticated already.send SetAliasIQ now...");
					// 组装IQ
